...
class VM(ABC):
    ...
    @abstractmethod
    def show_instance_public_dns(self):
        pass
...
class EC2(VM):
    def __init__(self, instance_id, instance_type, instance_state, public_dns_name, tags):
    ...
    def restart_instance(self):
        ec2_client.reboot_instances(InstanceIds=[self.instance_id])
...
class EC2Cluster:
    ...
    def start_all(self):
        for instance in self.instances:
            ...
            public_dns_name = instance.show_instance_public_dns()
            if instance.tags[0]['Value'] == "ford-pmm-server":
                print("ford-pmm-server開機了")
                try:
                    update_ssh_config(
                        target_host="prometheus-server", new_hostname=public_dns_name)
                    print(f"{public_dns_name} change to get new prometheus-server")
                except Exception as e:
                    print(e)
            elif instance.tags[0]['Value'] == 'ford-pmm-test':
                print('ford-pmm-test開機了')
                try:
                    update_ssh_config(target_host="backend1",
                                      new_hostname=public_dns_name)
                    print(f"{public_dns_name} change to get new backend1")
                except Exception as e:
                    print(e)
            else:
    ...
    def stop_all(self):
        for instance in self.instances:
            ...
            print(f"{instance.instance_id}被stop了")
        return
...
def get_instances(ec2_client_side):
    ...
    for reservation in response['Reservations']:
        for instance in reservation['Instances']:
            ...
            ec2_instance = EC2(instance_id, instance_type, instance_state,
                               instance_public_dns_name, instance_tags)

            instances.append(ec2_instance)
    return EC2Cluster(instances)


def update_ssh_config(target_host: str, new_hostname):
    config_file = '/Users/user/.ssh/config'  # SSH 配置文件路径
    ...
    if target_index != -1:
        ...
        lines[target_index + 1] = f'    HostName {new_hostname}\n'
